<?php
    ...
    {
        try{
            $s = new PDO('mysql:host=localhost;dbname=test', "root", "",array(PDO::ATTR_ERRMODE => PDO::ERRMODE_WARNING));
            return $s;
    ...
    function consultar($id)
    {
        ...
        try{
            $query = sprintf("SELECT * FROM imagen WHERE id_imagen=%d LIMIT 1",(int)$id);
            $result = $conn->query($query);
    ...
    function obtenerRegistros($datos) 
    {
        ...
        if ($login['code']=="0")
        {
            $respuesta['datos'] = "Te has logeado";
            $respuesta['codigo'] = "0";
            $respuesta['mensaje'] = "";
        }
    ...
    function obtener_img($datos) 
    {
        ...
        if ($login['code']=="0")
        {
            $respuesta['datos'] = consultar($datos['id']);
            $respuesta['codigo'] = "0";
            $respuesta['mensaje'] = "";
        }
        else if ($login['code']=="-1")
    ...
    $server->register("obtener_img",
        array("datos" => "xsd:string"),
        array("return" => "xsd:string"),
        "urn:registros",
        "urn:registros#obtener_img",
        "rpc",
        "encoded",
        "Propociona los registros de una tabla");
